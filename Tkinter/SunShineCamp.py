...
def print_camp_details():
    ...
    while name_count < total_entries:
        ...
        Label(frame, text=(camp_details[name_count][0])).grid(
            column=1, row=name_count + 8
        )
        Label(frame, text=(camp_details[name_count][1])).grid(
            column=2, row=name_count + 8
        )
        Label(frame, text=(camp_details[name_count][2])).grid(
            column=3, row=name_count + 8
        )
        Label(frame, text=(camp_details[name_count][3])).grid(
...
def setup_buttons():
    ...
    # Title
    Label(main_window, text="Sunshine Adventure", font=(("Aerial"), 12)).grid(
        column=0, row=0
    )
...
def check_validity():
    ...
    Label(main_window, text="Weather").grid(column=0, row=5)
    if (
        len(entry_leader.get()) != 0
        and entry_campers.get().isdigit()
        and len(entry_weather.get()) != 0
        and len(entry_location.get()) != 0
    ):
        if int(entry_campers.get()) >= 5 and int(entry_campers.get()) <= 10:
            append_name()
            print_camp_details()
        else:
            messagebox(main_window, text = "Number of Campers", bg = "red").grid(column = 0, row = 4 )
            

    else:
        if len(entry_leader.get()) == 0:
            Label(main_window, text="Leader", bg="red").grid(column=0, row=2)
        if len(entry_location.get()) == 0:
            Label(main_window, text="Location", bg="red").grid(column=0, row=3)
        if len(entry_weather.get()) == 0:
            Label(main_window, text="Weather", bg="red").grid(column=0, row=5)
        if entry_campers.get().isdigit():
            if (
                len(entry_campers.get()) == 0
                or int(entry_campers.get()) < 5
                or int(entry_campers.get()) > 10
            ):
                Label(main_window, text="Number of Campers", bg="red").grid(
                    column=0, row=4
                )
        else:
